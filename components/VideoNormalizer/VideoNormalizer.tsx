...
function VideoNormalizer() {
  let imageEditorViewContainerRef: MutableRefObject<HTMLDivElement | null> =
    useRef(null);
  let cameraViewContainerRef: MutableRefObject<HTMLDivElement | null> =
    useRef(null);
  let normalizedImageContainer: MutableRefObject<HTMLDivElement | null> =
    useRef(null);
  let cameraEnhancer: MutableRefObject<Promise<CameraEnhancer> | null> =
    useRef(null);
  let router: MutableRefObject<Promise<CaptureVisionRouter> | null> =
    useRef(null);
    ...
    const init = async () => {
      try {
        ...
        imageEditorView.current = await ImageEditorView.createInstance(
          imageEditorViewContainerRef.current as HTMLDivElement
        );
        ...
        resultReceiver.onCapturedResultReceived = (result) => {
          ...
          if (originalImage.length) {
            image.current = (
              originalImage[0] as OriginalImageResultItem
            ).imageData;
          }
          items.current = result.items.filter((item: CapturedResultItem) => {
        ...
        await normalizer.current.startCapturing(
          "DetectDocumentBoundaries_Default"
        );
  ...
  const confirmTheBoundary = () => {
    ...
    for (let i = 0; i < items.current.length; i++) {
      if (
        items.current[i].type === EnumCapturedResultItemType.CRIT_ORIGINAL_IMAGE
      )
        continue;
  ...
  const normalize = async () => {
    ...
    let newSettings = (await normalizer.current!.getSimplifiedSettings(
      "normalize-document"
    )) as SimplifiedCaptureVisionSettings;
    ...
    let norRes = await normalizer.current!.capture(
      image.current!,
      "normalize-document"
    );
    if (norRes.items[0]) {
      normalizedImageContainer.current!.append(
        (norRes.items[0] as NormalizedImageResultItem).toCanvas()
      );
    }
  ...
  return (
    <div className="container">
      <div
        id="div-loading"
        style={{ display: bShowLoading ? "block" : "none" }}
      >
        Loading...
      </div>
